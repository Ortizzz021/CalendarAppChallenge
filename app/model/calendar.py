...
class Event:
    ...
    def add_reminder(self, data_time: datetime, type_: str):
        reminder = Reminder(data_time)
...
class Calendar:
    ...
    def add_event(self, title: str, description: str, date_: date, start_at: time, end_at: time) -> str:
        if date_ < datetime.now().date():
            ...
            if date_ is not self.days.keys():
                day = Day(date_)
                self.days[date_] = day
    ...
    def add_reminder(self, event_id: str, date_time: datetime, type_: str):
        if event_id is not self.events.keys():
            event_not_found_error()
        else:
            self.add_reminder(event_id, date_time, type_)
